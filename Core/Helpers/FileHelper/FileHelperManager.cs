...
namespace Core.Helpers.FileHelper
{
    public class FileHelperManager : IFileHelper
    {
        ...
        {
            if (File.Exists(filePath))//eğer dosya var ise dosyayı bulunduğu yerden sil
            {
        ...
        public string Upload(IFormFile file, string root)
        {
            ...
            if (file.Length > 0)
            {
                ...
                string guid = GuidHelperManager.CreateGuid();//GuidHelper sınıfındaki yazdığımız methodu kullanarak eşsiz isim atadık.
                string filePath = guid + extension;//dosya adı+dosya uzantısı, örneğin drsPicture.jpg
